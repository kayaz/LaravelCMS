                ...
                <table class="table mb-0" id="sortable">
                    ...
                    @foreach ($list as $index => $p)
                        <tr id="recordsArray_{{ $p->id }}">
                            ...
                            <td><a href="{{route('admin.gallery.pokaz', ['id' => $p->id])}}" data-toggle="tooltip" data-placement="top" title="Pokaż katalog">{{ $p->nazwa }}</a></td>
                            <td>{{ $p->updated_at }}</td>
                            <td class="option-120">
                                <div class="btn-group">
                                    <a href="{{route('admin.gallery.pokaz', ['id' => $p->id])}}" class="btn action-button mr-1" data-toggle="tooltip" data-placement="top" title="Pokaż katalog"><i class="fe-folder"></i></a>
                                    <a href="{{route('admin.gallery.edytuj', ['id' => $p->id])}}" class="btn action-button mr-1" data-toggle="tooltip" data-placement="top" title="Edytuj katalog"><i class="fe-edit"></i></a>
                                    <form method="POST" action="{{route('admin.gallery.usun', ['id' => $p->id])}}">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
