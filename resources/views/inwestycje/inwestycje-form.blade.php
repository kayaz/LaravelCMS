@extends('admin')
@section('content')
    @if(Route::is('admin.inwestycje.edytuj'))
        <form method="POST" action="{{route('admin.inwestycje.update', $wpis->id)}}" enctype="multipart/form-data">
            @method('PUT')
    @else
        <form method="POST" action="{{route('admin.inwestycje.zapisz')}}" enctype="multipart/form-data">
    @endif
    @csrf
