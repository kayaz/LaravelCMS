...
@section('content')
                ...
                <div class="card thumb-card">
                    ...
                    <div class="card-body">
                        <div class="btn-group">
                            ...
                            <form method="POST" action="{{route('admin.galeria.usunzdjecie', ['id' => $p->id, 'id_gal' => $p->id_gal])}}">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
    ...
    <script type="text/javascript" language="javascript">
        $(window).on('shown.bs.modal', function() { $('#bootstrapmodal').modal('show'); var fileCount = 0; $('#jquery-wrapped-fine-uploader').fineUploader({debug: true, request: {endpoint: '{{route('admin.galeria.upload', ['id' => $id])}}', customHeaders: {"X-CSRF-Token": $("meta[name='csrf-token']").attr("content")}}}).on('error', function(event, id, name, reason) {}).on('submit', function(id, nameN){fileCount++;}).on('complete', function(event, id, name, response){if(response.success==true){fileCount--;if(fileCount == 0){location.reload();}}});});
        $(document).ready(function(){$("#sortable").sortujGal('{{route('admin.galeria.sort')}}');});
    </script>
@endsection
@section('scripts')
