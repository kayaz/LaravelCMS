@extends('admin')
@section('content')
    @if(Route::is('admin.boks.edytuj'))
        <form method="POST" action="{{route('admin.boks.update', $wpis->id)}}" enctype="multipart/form-data">
        {{method_field('PUT')}}
    @else
        <form method="POST" action="{{route('admin.boks.zapisz')}}" enctype="multipart/form-data">
    @endif
        @csrf
